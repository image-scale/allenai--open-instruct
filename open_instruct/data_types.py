"""Data container classes for the training pipeline."""
...
class RequestInfo:
    ...
    """Per-sample rollout state dicts (rewards, step_count, done, info)."""


@dataclass
class GenerationResult:
    """Container for generation results returned via queues."""
...
@dataclass
class PromptRequest:
    """Container for prompt requests sent via queues.

    Note: We intentionally type `generation_config` as `Any` to avoid importing
    heavy dependencies (e.g., vLLM) at import time.
    """
    ...
    """Optional ground truth override (e.g. from evolving rubrics)."""
...
class CollatedBatchData:
    ...
    def __getitem__(self, idx: int | slice) -> "CollatedBatchData":
        """Index into the batch to get a subset."""
    ...
    def to(
        ...
        return dataclasses.replace(
            ...
            **{
                f.name: [
                    t.to(device, non_blocking=non_blocking) for t in getattr(self, f.name)
                ]
